...
export default function Page({
  ...
  useEffect(() => {
    fetch(
      `/api/fund/transactionSet/${transactionSetId}/basicInfoUnitPricesDividendsSplitsTransactions`
    ).then(async (res) => {
  ...
  // )
  //
  // const transactionSetActive = useMemo(
  //   () =>
  //     transactionSet && transactionSet.status === TransactionSetStatus.Active,
  //   [transactionSet]
  // )
  //
  // // 获取爬虫的四大基本数据
  // const { data: fourBasicData } = useRequest(
  //   async () => {
  ...
  //
  // // 获取交易记录
  // const { data: transactions } = useRequest(
  //   async () => {
  //     if (!transactionSet?.target) {
  //       return
  //     }
  ...
  // >([])
  // const [rateOfReturnChartData, setRateOfReturnChartData] = useState<
  //   { date: string; type: string; rate: number }[]
  // >([])
  // const [annualizedRateOfReturnChartData, setAnnualizedRateOfReturnChartData] =
  //   useState<{ date: string; type: string; rate: number }[]>([])
  //
  ...
  //   ) => {
  //     const isArchivedSet =
  //       inputTransactionSet.status === TransactionSetStatus.Archived
  //     const formattedUnitPrices = sliceBetween(
  //       inputUnitPrices,
  //       inputTransactions[0].date,
  //       isArchivedSet ? lastOfArray(inputTransactions).date : dayjs()
  //     )
  ...
  //       const { unitCost, totalRateOfReturn, totalAnnualizedRateOfReturn } =
  //         await new Promise<{
  //           unitCost: number
  ...
  //   }
  //   const isArchivedSet =
  //     transactionSet.status === TransactionSetStatus.Archived
  //   const formattedUnitPrices = sliceBetween(
  //     fourBasicData.unitPrice,
  //     transactions[0].date,
  //     isArchivedSet ? lastOfArray(transactions).date : dayjs()
  //   )
  //   return calcReturn(
  //     sliceBetween(
  //       formattedUnitPrices,
  //       formattedUnitPrices[0].date,
  //       lastOfArray(formattedUnitPrices).date
  //     ),
  //     sliceBetween(
  //       fourBasicData.dividend,
  //       formattedUnitPrices[0].date,
  ...
  //       <Overview
  //         backgroundColor={
  //           overviewData.totalAnnualizedRateOfReturn > 0
  //             ? COLOR.Profitable
  //             : COLOR.LossMaking
  //         }
  //         data={[
  ...
  //               ? lastOfArray(fourBasicData.unitPrice)
  ...
  //           [
  //             '收益率',
  //             formatToCurrency(overviewData.totalRateOfReturn * 100) + '%',
  //           ],
  //           [
  //             '年化收益率',
  //             formatToCurrency(overviewData.totalAnnualizedRateOfReturn * 100) +
  //               '%',
  //           ],
  //         ]}
  //       />
  ...
  //               onClick={() => {
  //                 history.push(
  //                   `/fund/transaction?targetFund=${transactionSet?.target}`
  //                 )
  //               }}
  //             />
  //           )}
